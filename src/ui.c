...
void ui_draw_button(UI_Box *e)
{
	...
	if(e->theme.background.enabled)
	{
		...
		background = fvec4_add(background, e->theme.states[e->state.pressed_ptr[0]].color);
...
UI *poll_ui_test(UI *ui, FrameEvents fe, uvec2 frame_size, BoidSim *sim)
{
	...
	struct GlobalBoidParams *bp = &THREAD->global.boid;
	{
		...
		u32 top_y = 0;
		{
			...
			// This is convoluted
			// Trying to get the button light up also when a key is pressed
			if(fe.b.pressed != false)
			{
				bp->bump_enable = true;
			}
			if(fe.b.release_time == fe.time)
			{
		...
		}
		{
			...
			f32 x1 = x0 + width;
			f32 y1 = y0 + size;
			UI_Button *controls_button = ui_button(box, "Controls", x0,y0,x1,y1);
			...
			if(controls_button->state.hovering || bp->show_controls)
			{
				...
				u32 width = 400;
				u32 height = 640 - (bottom_right.y - top_left.y);
...

		{
			...
			count_box->text = string_print(ui->arena,
				"Boid Count:   %u32K\n"
				"Thread Count: %u32 of %u32 (static)\n"
				,bp->boid_count_uint / 1024,sim->max_thread_count, THREAD_COUNT);
...

		{
			...
			f32 x1 = x0 + width;
			f32 y1 = y0 + size;
			UI_Slider *count_slider= ui_slider(box,0, x0,y0,x1,y1);
			count_slider->text = str8_lit("Boid Count (Auto Reset)");

			// BUG:
			// Both norm and ptr neeed to be set because otherwise they will be reset when a pannel is shown conditionally.
			count_slider->state.norm = bp->boid_count_norm;
			count_slider->state.norm_ptr= &bp->boid_count_norm;
			bp->boid_count_uint = (u32)(powf(count_slider->state.norm, 2.0) * (f32)(sim->max_boid_count));
			bp->boid_count_uint = forward_align_uint(bp->boid_count_uint, 1024 * 32);
			bp->boid_count_uint = defmax(1024 * 32, bp->boid_count_uint);
			bp->boid_count_uint = defmin(bp->boid_count_uint, sim->max_boid_count);
			if(bp->boid_count_uint != sim->boid_count)
			{
				reset_boid_sim(sim);
			}
			left_x += width;
			count_slider->theme.corner_radius = 2.0;
		}
